        ...
        private static FilterDefinition<BsonDocument> StringQuery(string parameterName, Operator optor, string modifier, ValueExpression operand)
        {
            switch (optor)
            {
                ...
                case Operator.ISNULL:
                    return Builders<BsonDocument>.Filter.Or(Builders<BsonDocument>.Filter.Not(parameterName), Builders<BsonDocument>.Filter.Eq(parameterName, BsonNull.Value)); //With only Builders<BsonDocument>.Filter.NotExists, that would exclude resources that have this field with an explicit null in it.
                ...
                case Operator.EQ:
                    ...
                    else //Search on code and system
                    {
                        ...
                        if (!typedEqOperand.AnyNamespace)
                        {
                            ...
                            {
                                arrayQueries.Add(Builders<BsonDocument>.Filter.Not("system"));
                                noArrayQueries.Add(Builders<BsonDocument>.Filter.Not(systemfield));
                            }
                            else
                            {
