...
namespace Spark.Search.Mongo
{
    ...
    public class MongoSearcher
    {
        ...
        private List<BsonValue> CollectKeys(FilterDefinition<BsonDocument> query)
        {
            ...
            IEnumerable<BsonDocument> cursor = _collection.Find(query).Project(Builders<BsonDocument>.Projection.Include(InternalField.ID)).ToEnumerable();
            if (cursor.Count() > 0)
        ...
        private SearchResults KeysToSearchResults(IEnumerable<BsonValue> keys)
        {
            ...
            if (keys.Count() > 0)
            {
                IEnumerable<BsonDocument> cursor = _collection.Find(Builders<BsonDocument>.Filter.In(InternalField.ID, keys)).Project(Builders<BsonDocument>.Projection.Include(InternalField.SELFLINK)).ToEnumerable();
        ...
        private static SortDefinition<BsonDocument> CreateSortBy(IList<Tuple<string, SortOrder>> sortItems)
        {
            ...
            SortDefinition<BsonDocument> builder = null;
            var first = sortItems.FirstOrDefault();
            if(first.Item2 == SortOrder.Ascending)
            {
                builder = Builders<BsonDocument>.Sort.Ascending(first.Item1);
            }
            else
            {
                builder = Builders<BsonDocument>.Sort.Descending(first.Item1);
            }
