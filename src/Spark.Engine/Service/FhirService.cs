...
namespace Spark.Engine.Service
{
	...
	//A future step might be to split that part into a different service (maybe StorageService?)
	{
		...
		{
			var conformanceService = this.GetFeature<ICapabilityStatementService>();

			return Respond.WithResource( conformanceService.GetSparkCapabilityStatement( sparkVersion ) );
		}
