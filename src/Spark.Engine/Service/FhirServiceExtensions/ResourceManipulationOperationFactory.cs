...
namespace Spark.Engine.Service.FhirServiceExtensions
{
    ...
    {
        private static Dictionary<Bundle.HTTPVerb, Func<Resource, IKey, ISearchService, SearchParams, System.Threading.Tasks.Task<ResourceManipulationOperation>>> builders;
        private static ISearchService searchService;

        static ResourceManipulationOperationFactory()
        {
            builders = new Dictionary<Bundle.HTTPVerb, Func<Resource, IKey, ISearchService, SearchParams, System.Threading.Tasks.Task<ResourceManipulationOperation>>>();
        ...
        public static async Task<ResourceManipulationOperation> CreatePost(Resource resource, IKey key,
            ISearchService service = null, SearchParams command = null)
        {
            searchService = service;
            return new PostManipulationOperation(resource, key, await GetSearchResult(key, command), command);
        }

        public static async Task<ResourceManipulationOperation> CreatePut(Resource resource, IKey key,
            ISearchService service = null, SearchParams command = null)
        {
            searchService = service;
            return new PutManipulationOperation(resource, key, await GetSearchResult(key, command), command);
        }

        public static async Task<ResourceManipulationOperation> CreateDelete(IKey key, ISearchService service = null,
            SearchParams command = null)
        {
            searchService = service;
            return new DeleteManipulationOperation(null, key, await GetSearchResult(key, command), command);
        }

        private static async Task<ResourceManipulationOperation> CreateDelete(Resource resource, IKey key,
            ISearchService service = null, SearchParams command = null)
        {
            searchService = service;
            return new DeleteManipulationOperation(null, key, await GetSearchResult(key, command), command);
        ...
        public static async Task<ResourceManipulationOperation> GetManipulationOperation(
            Bundle.EntryComponent entryComponent, ILocalhost localhost, ISearchService service = null)
        {
            searchService = service;
            ...
            var search = searchUri != null ? ParseQueryString(localhost, searchUri) : null;
            var result = await builders[method](entryComponent.Resource, key, service, search);
            return result;
        }
