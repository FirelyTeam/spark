...
namespace Spark.Engine.Service
{
    public class AsyncFhirService : AsyncFhirServiceBase, IAsyncInteractionHandler
    {
        ...
        {
            var storageService = GetFeature<IAsyncResourceStorageService>();
        ...
        {
            var searchStore = GetFeature<IAsyncSearchService>();
        ...
        public override async Task<FhirResponse> DeleteAsync(IKey key)
        {
            ...
            var resourceStorage = GetFeature<IAsyncResourceStorageService>();
        ...
        {
            var pagingExtension = GetFeature<IAsyncPagingService>();
        ...
        {
            var historyExtension = GetFeature<IAsyncHistoryService>();
        ...
        public override async Task<FhirResponse> ReadAsync(IKey key, ConditionalHeaderParameters parameters = null)
        {
            ...
            var entry = await GetFeature<IAsyncResourceStorageService>().GetAsync(key).ConfigureAwait(false);
        ...
        {
            var searchService = GetFeature<IAsyncSearchService>();
        ...
        public override async Task<FhirResponse> VersionSpecificUpdateAsync(IKey versionedKey, Resource resource)
        {
            ...
            var current = await GetFeature<IAsyncResourceStorageService>().GetAsync(key).ConfigureAwait(false);
        ...
        {
            var pagingExtension = FindExtension<IAsyncPagingService>();
            if (pagingExtension == null)
            {
                ...
                var resourceStorage = FindExtension<IAsyncResourceStorageService>();
